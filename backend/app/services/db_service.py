...
def create_portfolio(db: Session, user_id: UUID, portfolio: PortfolioCreate) -> models.Portfolio:
...
def get_portfolio(db: Session, portfolio_id: UUID) -> Optional[models.Portfolio]:
    """Get portfolio by ID"""
    return db.query(models.Portfolio).filter(models.Portfolio.id == portfolio_id).first()


def get_user_portfolios(db: Session, user_id: UUID) -> List[models.Portfolio]:
    """Get all portfolios for a user"""
    return db.query(models.Portfolio).filter(models.Portfolio.user_id == user_id).all()


def update_portfolio(db: Session, portfolio_id: UUID, updates: dict) -> models.Portfolio:
    """Update portfolio"""
...
def create_or_update_profile(db: Session, portfolio_id: UUID, profile: ProfileDataCreate) -> models.ProfileData:
    ...
    if db_profile:
        ...
        db_profile = models.ProfileData(portfolio_id=portfolio_id, **profile.dict(exclude_unset=True))
        db.add(db_profile)
...
def create_profile_data(db: Session, portfolio_id: UUID, profile: ProfileDataCreate) -> models.ProfileData:
    """Alias for create_or_update_profile - Create or update profile data"""
    return create_or_update_profile(db, portfolio_id, profile)


def get_profile(db: Session, portfolio_id: UUID) -> Optional[models.ProfileData]:
    """Get profile data"""
...
def create_project(db: Session, portfolio_id: UUID, project: ProjectCreate) -> models.Project:
...
def get_projects(db: Session, portfolio_id: UUID) -> List[models.Project]:
...
def create_skill(db: Session, portfolio_id: UUID, skill: SkillCreate) -> models.Skill:
    """Create a new skill"""
...
def get_skills(db: Session, portfolio_id: UUID) -> List[models.Skill]:
    """Get all skills for a portfolio"""
    return db.query(models.Skill).filter(models.Skill.portfolio_id == portfolio_id).all()


def bulk_create_skills(db: Session, portfolio_id: UUID, skills: List[SkillCreate]) -> List[models.Skill]:
    """Create multiple skills at once"""
    db_skills = [
        models.Skill(portfolio_id=portfolio_id, **skill.dict())
        for skill in skills
    ]
...
def create_social_link(db: Session, portfolio_id: UUID, link: SocialLinkCreate) -> models.SocialLink:
...
def get_social_links(db: Session, portfolio_id: UUID) -> List[models.SocialLink]:
...
def bulk_create_social_links(db: Session, portfolio_id: UUID, links: List[SocialLinkCreate]) -> List[models.SocialLink]:
    """Create multiple social links at once"""
    db_links = [
        models.SocialLink(portfolio_id=portfolio_id, **link.dict())
        for link in links
    ]
    db.add_all(db_links)
    db.commit()
