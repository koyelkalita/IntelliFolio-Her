...
const PortfolioPage = ({
  ...
  return (
    <div className="bg-foreground text-background geist-font">
      {showAnimatedBackground && <AuroraBackground />}
      <div className="relative">
        <nav className="w-full px-6 py-6 bg-foreground/30 backdrop-blur-md fixed top-0 left-0 z-10">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center space-x-2">
              <div className="w-8 h-8 rounded-lg bg-border backdrop-blur-md border border-border flex items-center justify-center">
                <span className="geist-font text-sm font-bold text-background">
                  {logo.initials}
                </span>
              </div>
              <span className="geist-font text-lg font-medium text-background">
                {logo.name}
              </span>
            </div>
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="text-muted-foreground hover:text-background transition-colors inter-font text-sm"
                >
                  {link.label}
                </a>
              ))}
            ...
            <button
              ...
              className="glass-button px-4 py-2 rounded-lg text-background text-sm font-medium inter-font"
            >
              {resume.label}
            </button>
          </div>
        ...
        <main
          id="about"
          className="w-full min-h-screen flex flex-col items-center justify-center px-6 py-28"
        >
          <div className="max-w-6xl mx-auto text-center">
            <div className="mb-8 float-animation">
              <h1 className="md:text-6xl lg:text-7xl leading-[1.1] geist-font text-5xl font-light text-background tracking-tight mb-4">
                {hero.titleLine1}
                <span className="text-background block tracking-tight">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
              <button
                ...
                className="primary-button px-6 py-3 text-background rounded-lg font-medium text-sm min-w-[160px]"
              >
                {ctaButtons.primary.label}
              </button>
              <button
                onClick={ctaButtons.secondary.onClick}
                className="glass-button min-w-[160px] inter-font text-sm font-medium text-background rounded-lg px-6 py-3"
              >
            ...
            >
              {projects.map((project, index) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-medium text-background mb-2 geist-font">
                    {project.title}
                  </h3>
            ...
            >
              {stats.map((stat, index) => (
                <React.Fragment key={stat.label}>
                  <div>
                    <div className="text-3xl md:text-4xl font-light text-background mb-1 geist-font tracking-tight">
                      {stat.value}
                    </div>
